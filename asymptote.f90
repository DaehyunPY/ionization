...
subroutine mat_f(j)
    ...
    do h = 0, L
        tmp1 = aimag(S(j, h, F))/2.0_dp
        tmp2 = (1.0_dp -real(S(j, h, F)))/2.0_dp
...
subroutine PROC_CS_plot
    ...
    character(30), parameter :: &
        form_out = '(1A15, 5X, 1ES25.10)', &
        form_cs  = '(2X, 30ES25.10)', &
        form_tit = '("# ", 1A50)', &
        form_sub = '("# ", 2A25, 1A50)', &
...
subroutine PROC_E_vs_CS_plot
    ...
    write(file_cs, form_tit) "=================================================="
    write(file_cs, form_sub) " ENERGY ", " CROSS SECTION ", " MAXIMUM CROSS SECTION ", " RATE TO MAXIMUM "
    write(file_cs, form_sub) " ----------------------- ", " ----------------------- ", &
                             " ----------------------- ", " ----------------------- "
    write(file_cs, *)
    if(.not. allocated(outer_f)) allocate(outer_f(0:L))
    do j = 1, M
        ...
        tmp1  = sum1
        tmp2  = sum2
        write(file_cs, form_cs) coord_E(j)*unit_e, aimag(tmp1)*unit_cs, tmp2*unit_cs, aimag(tmp1)/tmp2
    end do
    if(allocated(outer_f)) deallocate(outer_f)
    close(file_cs)
...
subroutine PROC_E_vs_PS_plot
    ...
    character(30), parameter :: &
        form_out = '(1A15, 1I15, 2ES15.3)', &
        form_gen = '(2X, 1I5, 2ES25.10)', &
        form_tit = '("# ", 1A50)', &
        form_sub = '("# ", 3A25)'
    real(dp), parameter   :: au_to_eV = au_hartree/other_e_ev
    ...
    write(file_ph, form_tit) "=================================================="
    write(file_ph, form_sub) " ANGULAR MOMANTUM ", " ENERGY ", " PHASE SHIFT "
    write(file_ph, form_sub) " ----------------------- ", " ----------------------- ", " ----------------------- "
    write(file_ph, *)
    do i = 0, L
        do j = 1, M
    ...
    write(file_lt, form_tit) "=================================================="
    write(file_lt, form_sub) " ANGULAR MOMANTUM ", " ENERGY ", " LIFT-TIME "
    write(file_lt, form_sub) " ----------------------- ", " ----------------------- ", " ----------------------- "
    write(file_lt, *)
