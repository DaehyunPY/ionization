module global 
    use kind_type 
   implicit none
    integer (i1), parameter :: file_input = 5, file_log = 6 
    real    (dp), save :: Amp, Freq, Mass, Charge, Spin, Scatt, ra, dr_p_drho
    integer (i4), save :: L, N, F, M, pr, ptheta 
    real    (dp), save, allocatable :: coord_rho(:), coord_weight(:), coord_dshape(:, :), E(:)
    complex (dp), save, allocatable :: H(:, :), R(:), K(:), S(:), A(:)
    character(1), save :: &
        op_ev, op_degree, op_aa, & 
        op_poten, op_basis, op_dcs, op_inner, op_outer, &
        op_tcs, op_phase, op_lt
end module
