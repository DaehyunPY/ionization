...
subroutine PROC_input
    ...
    character(90), parameter :: &
        form_opt   = '(6/, 3(45X, 6X, 1A1, /), 1/, 5(45X, 6X, 1A1, /), 3/, 3(45X, 6X, 1A1, /))'
    ...
    if(Mass <  0_dp)   stop "Input value is not valid: check 'PARTICLE - MASS'."
    if(Spin /= 0.5_dp) stop "Input value is not valid: check 'PARTICLE - SPIN'."
    if(tmp1 >= 0_dp .and. tmp2 >= 0_dp) stop "Input value is not valid: check 'PARTICLE - SCATTERING ENERGY'."
    if(tmp1 <  0_dp .and. tmp2 <  0_dp) stop "Input value is not valid: check 'PARTICLE - SCATTERING ENERGY'."
    if(tmp1 >= 0_dp) Scatt = tmp1 
    if(tmp2 >= 0_dp) Scatt = tmp2*eV_to_au
    ...
    if(ty == 0) then 
        ...
        if(pD < 0_dp .and. pH < 0_dp) stop "Input value is not valid: check 'POTENTIAL - COEFFICIENT D, H'."
        if(pD < 0_dp)     pD     = pH/Z**0.4_dp 
        if(pH < 0_dp)     pH     = pD*Z**0.4_dp 
        if(pdelta < 0_dp) pdelta = (alphab/(2_dp*Z**(1_dp/3_dp)))**(0.25_dp)
    else if(ty == 4) then 
    ...
    if(op_phase  /= "Y" .and. op_phase  /= "N") stop "Input value is not valid: check 'OPTION - KINETIC ENERGY vs PHASE'."
    if(op_lt     /= "Y" .and. op_lt     /= "N") stop "Input value is not valid: check 'OPTION - KINETIC ENERGY vs LIFETIME'."
    close(file_input) 
